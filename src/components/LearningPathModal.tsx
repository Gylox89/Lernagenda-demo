...
import KIQuizModal from './KIQuizModal';
...
const LearningPathModal: React.FC<LearningPathModalProps> = ({ isOpen, onClose }) => {
  ...
  // States für KI-Quiz
  const [showKIQuizModal, setShowKIQuizModal] = useState(false);
  const [quizTargetStatus, setQuizTargetStatus] = useState<'initial' | 'final'>('initial');
  const [pendingQuizItem, setPendingQuizItem] = useState<{ pathId: number; itemId: number; title: string } | null>(null);
  ...
  const startQuiz = (pathId: number, itemId: number, itemTitle: string, targetStatus: 'in-progress' | 'completed') => {
    setPendingQuizItem({ pathId, itemId, title: itemTitle });
    setQuizTargetStatus(targetStatus === 'in-progress' ? 'initial' : 'final');
                ...
                <div className="flex space-x-6 min-w-max">
                  {learningPaths.map((path) => (
                    ...
                    >
                      ...
                      <div className="p-4 h-96 overflow-y-auto scrollbar-thin">
  ...
  onClose={cancelQuiz}
  onComplete={handleQuizComplete}
  itemId={pendingQuizItem?.itemId || 1}
  itemTitle={pendingQuizItem?.title || ''}
  phase={quizTargetStatus} // 'initial' oder 'final'
/>
    </>
  );
};
